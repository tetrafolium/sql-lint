...
// Skipping because the travis build complains about a missing configuration
// file. Should work fine locally assuming you have the config file.
...
test("--port is a valid option", (done) => {
  shelltest().cmd(`${sqlLint} --help`).expect("stdout", /.*--port.*/).end(done);
});
