 ...
 */
class CheckerRunner {
  public run(sqlQueries: Query[], printer: Printer, prefix: string,
             omittedErrors: string[], database?: Database) {
    const checks =
        fs.readdirSync(__dirname + "/checks")
            .map((check) => { return path.parse(check).name; })
            .filter((item) => {
              const ignoredChecks = [
                "invalidOption",
                "tableNotFound",
    ...
    sqlQueries.forEach((query: any) => {
      ...
      if (content) {
        ...
        checks.forEach((check) => {
          ...
          // DB server checks
          if (checker.requiresConnection && database &&
